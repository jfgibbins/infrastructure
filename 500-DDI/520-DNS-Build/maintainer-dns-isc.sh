...
function unique_add_line()
{
  add_line="$1"
  filespec="$2"
  grep -q -c "${add_line}" "${BUILDROOT}${CHROOT_DIR}$filespec"
  retsts=$?
  if [ $retsts -ge 1 ]; then
    # not found
#    echo "# inserted by $(basename "$0")" >> "${BUILDROOT}${CHROOT_DIR}$filespec"
    echo "${1}" >> "${BUILDROOT}${CHROOT_DIR}$filespec"
...
RNDC_KEYNAME="rndc-key-rw"
RNDC_READ_KEYNAME="rndc-key-ro"
RNDC_PORT="953"
...
RNDC_READ_KEY_FILEPART="rndc"
RNDC_KEY_FILETYPE=".key"
RNDC_KEY_FILENAME="${RNDC_KEY_FILEPART}-${SERVER_NAME}-rw${RNDC_KEY_FILETYPE}"
RNDC_READ_KEY_FILENAME="${RNDC_READ_KEY_FILEPART}-${SERVER_NAME}-ro${RNDC_KEY_FILETYPE}"
...
LOG_DIRSPEC="/var/log/$LOG_SUB_DIRNAME"
...
RNDC_READ_KEY_FILESPEC="${RNDC_KEY_DIRSPEC}/$RNDC_READ_KEY_FILENAME"
...
INSTANCE_LOG_DIRSPEC="${LOG_DIRSPEC}"


  INSTANCE_RNDC_KEYNAME="rndc-${SERVER_NAME}-rw"
  INSTANCE_RNDC_READ_KEYNAME="rndc-${SERVER_NAME}-ro"
  INSTANCE_RNDC_CONF_DIRSPEC="${RNDC_CONF_DIRSPEC}"
  INSTANCE_RNDC_KEY_DIRSPEC="${INSTANCE_RNDC_CONF_DIRSPEC}/keys"
  INSTANCE_RNDC_CONF_FILESPEC="${RNDC_CONF_FILESPEC}"
  INSTANCE_RNDC_KEY_FILESPEC="${INSTANCE_RNDC_KEY_DIRSPEC}/$RNDC_KEY_FILENAME"
  INSTANCE_RNDC_READ_KEY_FILESPEC="${INSTANCE_RNDC_KEY_DIRSPEC}/$RNDC_READ_KEY_FILENAME"
...
if [ -n "$INSTANCE" ]; then
  ...
  INSTANCE_LOG_DIRSPEC="${LOG_DIRSPEC}/$INSTANCE"
  INSTANCE_RNDC_KEYNAME="rndc-${SERVER_NAME}-${INSTANCE}-rw"
  INSTANCE_RNDC_READ_KEYNAME="rndc-${SERVER_NAME}-${INSTANCE}-ro"
...
INSTANCE_RNDC_KEY_FILENAME="${RNDC_KEY_FILEPART}-${SERVER_NAME}-${INSTANCE}-rw${RNDC_KEY_FILETYPE}"
INSTANCE_RNDC_READ_KEY_FILENAME="${RNDC_READ_KEY_FILEPART}-${SERVER_NAME}-${INSTANCE}-ro${RNDC_KEY_FILETYPE}"
...
INSTANCE_RNDC_READ_KEY_FILESPEC="${INSTANCE_RNDC_KEY_DIRSPEC}/$INSTANCE_RNDC_READ_KEY_FILENAME"
...
DEFAULT_ZONE_DB_DIRNAME_A=("master", "slave", "hint", "mirror", "redirect", "stub")
DEFAULT_ZONE_DB_DIRNAME_A_ALT=("primary", "secondary", "hint", "mirror", "redirect", "stub")
...
ZONE_DB_DIRSPEC="${INSTANCE_VAR_LIB_NAMED_DIRSPEC}"
ZONE_DB_DIRNAME=("master" "slave" "hint" "mirror" "redirect" "stub")
ZONE_DB_DIRNAME_ALT=("primary" "secondary" "hint" "mirror" "redirect" "stub" "catalog")
...
INSTANCE_ZONE_DB_DIRSPEC="${ZONE_DB_DIRSPEC}"
...
if [ -n "$INSTANCE" ]; then
  ...
  INSTANCE_ZONE_DB_DIRSPEC="${ZONE_DB_DIRSPEC}${INSTANCE_SUBDIRPATH}"
...
PRIMARY_NAMED_CONF_FILENAME="masters-named.conf"
